...
const Sidebar: React.FC<SidebarProps> = ({ view, setView, chats, activeChatId, onSelectChat, onNewChat, user, onLogout }) => {
  return (
    <div className="w-72 bg-[#0d0d0d] border-r border-slate-900 flex flex-col h-full z-50">
      <div className="p-6">
        <div className="flex items-center gap-3 mb-10 px-2 cursor-pointer" onClick={() => setView('dashboard')}>
          ...
          <span className="text-xl font-black bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-500 uppercase tracking-tighter">Nexus</span>
        </div>
        ...
        <div className="space-y-1 overflow-y-auto custom-scrollbar max-h-64">
           {chats.map(chat => (
             <div 
              key={chat.id}
              onClick={() => onSelectChat(chat.id)}
              className={`p-3 rounded-xl cursor-pointer text-sm truncate font-medium transition-all ${activeChatId === chat.id ? 'bg-[#1a1a1a] text-indigo-400 border border-indigo-500/20' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`}
             >
               {chat.title}
             </div>
           ))}
      ...
      <div className="mt-auto p-6 bg-[#0a0a0a] border-t border-slate-900">
         ...
         <div className="text-[9px] text-slate-700 font-bold uppercase tracking-widest text-center">Nexus Enterprise v3.0</div>
      </div>
    </div>
  );
};
